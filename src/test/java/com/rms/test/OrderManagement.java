...
public class OrderManagement {
	...
	for(int i=2;i<=numberOfRowsInExcel;i++) {
	   ...
	   Thread.sleep(2000);
	   Thread.sleep(Integer.parseInt(WAITBETWEENPAGES));	
		List <WebElement> deleteAlreadyCartItems = driver.findElements(By.xpath("//button[@aria-label='Decrease quantity by one']"));
		...
		WebElement searchLensIcon = driver.findElement(By.id("nav-search-submit-button"));
...
//		}
		...
		WebElement cartIcon2 = driver.findElement(By.xpath("//span[@id='nav-cart-count']"));
		try {
			JavascriptExecutor je = ((JavascriptExecutor) driver);
			je.executeScript("arguments[0].scrollIntoView(true);",cartIcon2);
		}catch (Exception e) {
		...
		//  Then User click on to add new Address
		WebElement addANewAddress= driver.findElement(By.id("add-new-address-desktop-sasp-tango-link"));
...
//		((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", totalAddressCount);
		...
		WebElement phoneNumberEle = driver.findElement(By.id("address-ui-widgets-enterAddressPhoneNumber"));
		phoneNumberEle.click();
		phoneNumberEle.clear();
		String phoneNumberText = xlsReader.getCellData("order", 11, i).substring(0, 11);
		String phoneNumber = phoneNumberText.replaceAll("\\.", ""); 
		...
		WebElement sector = driver.findElement(By.id("address-ui-widgets-enterAddressLine2"));
		sector.click();
		sector.clear();
		sector.sendKeys(xlsReader.getCellData("order", 7, i));
		
		WebElement city = driver.findElement(By.id("address-ui-widgets-enterAddressLine2"));
		city.click();
		city.clear();
		city.sendKeys(xlsReader.getCellData("order", 7, i));
		
		
		WebElement useThisAddressButton = driver.findElement(By.id("address-ui-widgets-form-submit-button"));
		useThisAddressButton.click();
		Thread.sleep(5000);
		
		
		// Payment Options
		WebElement upiPaymentOption = driver.findElement(By.xpath("(//div[@class='a-fixed-left-grid-col a-col-left'])[4]"));
		upiPaymentOption.click();
		Thread.sleep(2000);
